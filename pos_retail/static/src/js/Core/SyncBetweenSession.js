odoo.define('pos_retail.synchronization', function (require) {
        ...
        { // TODO: for offline mode
            ...
            loaded: function (self, iot_boxes) {
                ...
                for (var i = 0; i < iot_boxes.length; i++) {
                    ...
                    iot_connection.rpc('/pos/passing/login', {})
                        .then(function (status) {
                            console.log('Result Ping POSBOX: ' + status)
                        }, function (error) {
                            console.error('Result Ping POSBOX error: ' + error)
                        })
    ...
    exports.pos_bus = Backbone.Model.extend({
        ...
        set_online: function () {
            ...
            setTimeout(() => {
                self.pos.set('syncStatus', {status: 'connected', pending: _t('')})
            }, 2000)
        },
        ...
        syncing() {
            ...
            async function waitSync() {
                if (!self.pos.config.sync_multi_session) {
                    console.log('Stopped Sync 5 seconds')
                    self.pos.set('syncStatus', {status: 'disconnected', pending: 'Stoped Syncing'});
                ...
                for (var i = 0; i < self.pos.iot_connections.length; i++) { //
                    ...
                    self.pos.set('syncStatus', {status: 'connected', pending: ''});
                    var params = {
                    ...
                    if (results) {
                        ...
                        if (notifications.length > 0) {
                            ...
                            const requests_printer = notifications.filter((n) => n[2]['action'] == 'request_printer')
                            const requests_transfer_succeed = notifications.filter((n) => n[2]['action'] == 'transfer_succeed_receipt')
                            let requests_normal = notifications.filter((n) => n[2]['action'] != 'request_printer' && n[2]['action'] != 'transfer_succeed_receipt')
                            ...
                            for (var i = 0; i < requests_transfer_succeed.length; i++) {
                                self.pos.get_notifications(requests_transfer_succeed[i][2])
                            }
                        }
                    }
                }
        ...
        send_notification: function (value, send_manual = false) {
            ...
            if (this.pos.session.restaurant_order) {
                console.log('{blocked sync from restaurant order}: ' + value.action)
                return true
            }
            if (!value.order_uid) {
            ...
            this.pos.set('syncStatus', {status: 'connecting', pending: ' Send: ' + value.action});
    ...
    models.PosModel = models.PosModel.extend({
        ...
        sync_lock_table: function (data) {
            ...
            var table_will_lock = _.find(this.pos.tables, function (tb) {
