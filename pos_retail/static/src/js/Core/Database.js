...
odoo.define('pos_retail.database', function (require) {
    ...
    _super_db.add_products = function (products) {
        ...
        for (var i = 0; i < products.length; i++) {
            ...
            if (product.pos_categ_ids && product.pos_categ_ids.length) {
                var base_categ_id = product.pos_categ_id ? product.pos_categ_id[0] : this.root_category_id;
            ...
            if (product.available_in_pos) {
                let categ_id
                if (self.posmodel.config.product_category_ids.length == 0) {
    ...
    db.include({
        ...
        _product_search_string: function (product) { // TODO: supported search with name_second
            ...
            let barcodes = self.posmodel.barcodes_by_product_id[product.id];
            ...
            let barcodesPackaging = self.posmodel.packaging_barcode_by_product_id[product.id];
        ...
        get_product_string: function (product) {
            ...
            if (product['lst_price']) {
                label += ', ' + self.posmodel.format_currency(product.lst_price);
            }
            return label
        },
        ...
        save_invoices: function (invoices) {
            ...
            for (var i = 0; i < invoices.length; i++) {
                ...
                if (invoice.deleted) {
                    ...
                    this.invoice_ids = this.invoice_ids.filter(move_id => move_id != invoice.id)
                    continue
                }
                ...
                if (!this.invoice_ids.includes(invoice.id)) {
        ...
        save_invoice_lines: function (lines) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (lines_invoice_by_id) {
                    lines_invoice_by_id = lines_invoice_by_id.filter(ml => ml.id != line.id)
                    if (!line.deleted) {
        ...
        getOrderReceiptByUid(uid) {
            var orderReceipts = this.load('orderReceipts', []);
        ...
        getQrCodeOrders: function () {
            var qrCodeOrders = this.load('qrCodeOrders', []);
            return qrCodeOrders
        },
