odoo.define('pos_retail.AccountInvoiceList', function (require) {
    ...
    class AccountMoveScreen extends PosComponent {
        ...
        patched() {
            posbus.on('save-account-move', this, this.reloadScreen);
        }
        ...
        reloadScreen() {
            if (this.state.selectedMove) {
                let moves = this.env.pos.db.get_invoices()
                let moveJustUpdated = moves.find(m => m.id == this.state.selectedMove.id)
                this.state.editModeProps = {
                    move: moveJustUpdated,
                };
        ...
        get getMoves() {
            const filterCheck = (move) => {
                if (this.filter && this.filter !== 'All Invoices') {
