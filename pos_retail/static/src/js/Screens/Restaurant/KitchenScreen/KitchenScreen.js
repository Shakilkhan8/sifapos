odoo.define('pos_retail.KitchenScreen', function (require) {
    ...
    class KitchenScreen extends IndependentToOrderScreen {
        ...
        newOrderReceiptsComing(receiptOrder) {
            console.log('{KitchenScreen.js} new order coming')
        ...
        reloadKitchenScreen() {
            console.log('{KitchenScreen.js} reloadKitchenScreen')
            this.clearSearch()
        }

        orderReceiptRemoved(vals) {
            console.log('{KitchenScreen.js} orderReceiptRemoved: ' + vals.uid)
        ...
        doneLine(event) { // method for kitchen only, them done each line receipt
            ...
            if (['New', 'Priority'].includes(line.state)) {
                line.state = 'Ready Transfer'
                line.note = this.env.pos.user.name + this.env._t(' set Ready Transfer')
                order.ready_transfer_items += line.qty_requested
                order.state = 'Ready Transfer'
            }
            const linesNotDone = order.new.find(n => ['New', 'Priority'].includes(n.state))
            if (!linesNotDone) { // check if have not any lines still New and Priority, set order to DONE
                this.state.orders = this.state.orders.filter(o => o.request_time != order.request_time)
                this.state.selectedOrder = null
            }
            this.removeAnotherLineTheSameOrderAndProduct(line)
            this.saveOrderReceipts()
            const receipt = this.state.orders.find(r => r.request_time == order.request_time)
