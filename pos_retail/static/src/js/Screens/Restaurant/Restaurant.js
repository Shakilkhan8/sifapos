odoo.define('pos_retail.restaurant', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        set_table: function (table, order) {
            ...
            if (this.order_to_transfer_to_different_table && table && this.pos_bus) {
                this.order_to_transfer_to_different_table.syncing = true
                this.pos_bus.send_notification({
                    action: 'order_transfer_new_table',
                    data: {
                        uid: this.order_to_transfer_to_different_table.uid,
                        table_id: table.id,
                        floor_id: table.floor_id[0],
                    },
                    order_uid: this.order_to_transfer_to_different_table.uid,
                });
                this.order_to_transfer_to_different_table.syncing = false
            }
            var order_of_this_table = this.get_order();
            if (this.config.required_set_guest && order_of_this_table && order_of_this_table.customer_count <= 1) {
                setTimeout(function () {
                    order_of_this_table.ask_guest()
                }, 1000)
            }
        },
        /// ======================== END FORECE =========================//
        ...
        get_notifications: function (message) {
            ...
            if (this.config.screen_type && this.config.screen_type == 'kitchen' && ['request_printer', 'transfer_succeed_receipt', 'paid_order', 'unlink_order'].indexOf(action) == -1) {
                console.log("{Restaurant.js} Reject SYNC, only action in list ['request_printer', 'line_removing', 'paid_order'] Can Sync")
                return
            }
            ...
            if (['request_printer', 'new_qrcode_order'].includes(action)) {
                this.play_sound();
            }
        ...
        sync_unlink_order: function (uid, action, user) {
            let res = _super_posmodel.sync_unlink_order.apply(this, arguments);
        ...
        sync_request_printer: function (vals) { // TODO: update variable set_dirty of line
            ...
            if (this.config.screen_type && (computeChanges.new.length > 0 || computeChanges.cancelled.length > 0)) {
                ...
                posbus.trigger('newOrderReceiptsComing', computeChanges);
            }
        },
    ...
    models.Order = models.Order.extend({
        ...
        buildReceiptKitchen(orderReceipt) {
            const self = this;
            const request_number = this.pos.db.getRequestKitchenNumber()
            // todo: request number and session_id of computeChanges is key unit of request print receipt
            ...
            orderReceipt['request_number'] = request_number
            ...
            if (orderReceipt.new.length > 0 || orderReceipt.cancelled.length > 0 && this.pos.config.sync_multi_session && this.pos.config.kitchen_screen) {
                ...
                // this.pos.chrome.showScreen('KitchenScreen', {
                //     selectedOrder: orderReceipt
                // });
            }
            return orderReceipt
        },
        saveChanges: function (locked_sync) { // TODO: set all line to Waiting and push event to pos kitchen screen
            if (locked_sync) {
                return _super_order.saveChanges.apply(this, arguments); // put back last state
            }
            var orderReceipt = this.buildReceiptKitchen(this.computeChanges());
