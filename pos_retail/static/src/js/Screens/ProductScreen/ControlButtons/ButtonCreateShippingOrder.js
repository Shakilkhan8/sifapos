        ...
        async onClick() {
            ...
            if (order.get_total_with_tax() <= 0 || order.orderlines.models.length == 0) {
                return this.showPopup('ConfirmPopup', {
                    title: _t('Alert'),
                    body: _t('Your shopping cart is empty, required Amount total bigger than 0'),
                })
            }
            if (!order.get_client()) {
            ...
            if (confirmed) {
                ...
                if (values.error) {
                    ...
                    const {confirmed, payload: response} = await this.showPopup('ErrorPopup', {
                        title: _t('Alert'),
                        body: values.error
                    })
                    if (confirmed) {
                ...
                if (client_id) {
                    ...
                    order.trigger('change', order);
                    let order_id = await this.env.pos.push_single_order(order, {
                        draft: true
                    })
