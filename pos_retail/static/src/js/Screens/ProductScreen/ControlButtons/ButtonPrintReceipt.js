    ...
    class ButtonPrintReceipt extends PosComponent {
        ...
        async _onClick() {
            ...
            if (this.env.pos.config.proxy_ip && this.env.pos.proxy.printer) {
                const printResult = this.env.pos.proxy.printer.print_receipt(qweb.render('XmlReceipt', this.env.pos.getReceiptEnv()));
                if (printResult.successful) {
                    this.showPopup('ConfirmPopup', {
                        title: this.env._t('Printed'),
                        body: this.env._t('Bill printed at your Printer')
                    })
                    return true;
                } else {
