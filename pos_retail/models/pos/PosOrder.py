...
class POSOrder(models.Model):
    ...
    booking_id = fields.Many2one(
        ...
        readonly=1)
    payment_journal_id = fields.Many2one('account.journal', string='Payment Journal', readonly=0, related=None, )
    ...
    def add_payment(self, data):
        ...
        _logger.info('PosOrder.py add_payment {}'.format(data))
        return res
    ...
    def create_from_ui(self, orders, draft=False):
        _logger.info('POSORDER.py push state with Draft {}'.format(draft))
        for o in orders:
            data = o['data']
    ...
    def _create_order_picking(self):
        ...
        hasPicking = (not self.session_id.update_stock_at_closing) or (
                    self.company_id.anglo_saxon_accounting and self.to_invoice)
        _logger.info('{PosOrder.py} has picking: %s' % hasPicking)
        return res
